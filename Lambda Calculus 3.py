...
def lambda_process(string_ST):
    ...
    while beta_BL:
        ...
        height_NT = 0
        beta_lambda_NT = 0
        while (string_ST.find("λ", beta_lambda_NT + 1) > -1) and not beta_BL:
            beta_lambda_NT = string_ST.find("λ", beta_lambda_NT + 1)
            DEBUG(debug_14, f"L260. beta_lambda_NT = {beta_lambda_NT}")
            DEBUG(debug_14, "L263. string_STΛ = " + string_ST[ : beta_lambda_NT] + "Λ" + string_ST[beta_lambda_NT + 1 : ])
            if (beta_lambda_NT >= 2):
                if (string_ST[beta_lambda_NT - 2] == "("):
                    beta_open_NT = beta_lambda_NT - 2
                    DEBUG(debug_14, f"L264. beta_open_NT = {beta_open_NT}")
                    beta_BL = True
                    for i in range(beta_lambda_NT):
                        if (string_ST[i] == "("):
                            height_NT += 1
                        elif (i > 0) and (string_ST[i - 1] == ")"):
                            height_NT -= 1
                    beta_height_2_NT = height_NT
                    beta_height_1_NT = height_NT - 1
                    DEBUG(debug_14, f"L273. beta_height_1_NT = {beta_height_1_NT}")
                    DEBUG(debug_14, f"L274. beta_height_2_NT = {beta_height_2_NT}")
        if beta_BL:
            height_NT = beta_height_1_NT - 1
            DEBUG(debug_8, "= = = = = = = = = = = = = = = = = = = =")            
            DEBUG(debug_4, f"L278. beta_height_1_NT = {beta_height_1_NT}")
            DEBUG(debug_8, f"L279. string_ST = {string_ST}")
            DEBUG(debug_8, f"L280. _________ = {beta_open_NT*'/' + string_ST[beta_open_NT : ]}")
            DEBUG(debug_8, f"L281. beta_open_NT = {beta_open_NT}")
            DEBUG(debug_8, f"L282. beta_lambda_NT = {beta_lambda_NT}")
            beta_dot_NT = -1
            beta_space_NT = -1            
            beta_close_NT = -1
            DEBUG(debug_8, f"L286. beta_space_NT = {beta_space_NT}")
            DEBUG(debug_8, f"L287. beta_close_NT = {beta_close_NT}")
            for i in range(beta_open_NT, len(string_ST)):
                DEBUG(debug_4, f"L289. i = {i} ; {string_ST[ : i]}")
                if (string_ST[i] == "("):
                    height_NT += 1
                    DEBUG(debug_4, f"L292. height_NT = {height_NT}")
                elif (i > 0) and (string_ST[i - 1] == ")"):
                    height_NT -= 1
                    DEBUG(debug_4, f"L295. height_NT = {height_NT}")
                if (string_ST[i] == ".") and (height_NT == beta_height_2_NT):
                    if (beta_space_NT == -1):
                        beta_dot_NT = i
                        DEBUG(debug_8 or debug_14, f"L299. beta_dot_NT = {beta_dot_NT}")
                elif (string_ST[i] == " ") and (height_NT == beta_height_1_NT):
                    beta_space_NT = i
                    DEBUG(debug_8 or debug_14, f"L302. beta_space_NT = {beta_space_NT}")
                elif (string_ST[i] == ")") and (height_NT == beta_height_1_NT):
                    beta_close_NT = i
                    DEBUG(debug_8 or debug_14, f"L305. beta_close_NT = {beta_close_NT}")
                    break
            beta_variable_ST = string_ST[beta_lambda_NT + 1 : beta_dot_NT]
            DEBUG(debug_6 or debug_13, f"L302. beta_variable_ST = {beta_variable_ST}")
            DEBUG(debug_13, "L303. string_ST = " + string_ST[ : beta_lambda_NT] + "Λ" + string_ST[beta_lambda_NT + 1 : ])
            ...
            while "__" in string_ST:
                string_ST = string_ST.replace("__", "_")
            DEBUG(debug_8, f"L316. string_ST = {string_ST}")
            string_ST = string_ST.replace("_", beta_replacer_ST)
            string_ST = string_ST.replace("((λ#.", "")
            string_ST = string_ST.replace(") *)", "")
            DEBUG(debug_8 or debug_9 or debug_13, f"L320. string_ST = {string_ST} <<<<<<<<<<<<<<<<<<<<<<<<<<<")
            DEBUG_input(debug_13, "OK? ")
            if not is_well_formed(string_ST):
                DEBUG(debug_13, f"L323. string_ST = {string_ST}")
                print("L324. ERROR: the string resulting from the latest beta-reduction is not well-formed.")
                quit()
            string_ST = alpha_replacement(string_ST)
    return string_ST
...
def alpha_replacement(string_ST):
    ...
    while alpha_BL:
        ...
        if alpha_BL:
            ...
            alpha_replacer_ST = ""
            while (alpha_replacer_ST == ""):
                alpha_replacer_ST = chr(random.randrange(65, 91)) + chr(random.randrange(65, 91)) + chr(random.randrange(65, 91))
                DEBUG(debug_15, f"A randomly-picked reserved keyword: {reserved_keywords_LSST[random.randrange(0, len(reserved_keywords_LSST))]}")
                if alpha_replacer_ST in reserved_keywords_LSST:
                    alpha_replacer_ST = ""
